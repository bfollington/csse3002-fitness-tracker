...
def calc_statistics(waypoints):
    ...
    for data in processed:
        ...
        if data[3] >= maxDist:
            ...
            maxDistTime = data[4]
        if data[4] != 0:
            totalKilojoules += calculate_kj_consumption(data[3]/data[4], float(data[4]) / 60, height, weight, age, male)
            speedGraph['x'].append(data[2] - processed[0][2])
            speedGraph['y'].append(data[3] / data[4])
    
    testKilojoules = calculate_kj_consumption(totalDist / totalTime, float(totalTime) / 60, height, weight, age, male)
